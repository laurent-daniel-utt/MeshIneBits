...
/**
 * A Generation is a set of solutions. Provides tools to reproduce solutions between them, select
 * the better ones, and mutate some (has to be used with genetics algorithms).
 *
 * @see Solution
 */
public class Generation {
  ...
  /**
   * A Generation is a set of solutions. Implements methods to sort, select, reproduce, mutate and
   * evaluate the solutions.
   *
   * @param popSize          the size of the population.
   * @param rankSelection    the percentage of selected solutions from the previous generation to
   *                         make the new one.
   * @param rankReproduction the percentage of reproduced solutions from the previous generation to
   *                         make the new one.
   ...
   */
  public Generation(int popSize, double rankSelection, double rankReproduction, double probMutation,
      int ratio, Area layerAvailableArea, Vector2 startPoint, Vector<Vector2> bound) {
  ...
  /**
   * Create a new random solution. Takes in account the <code>MAX_ANGLE</code> to generate the
   * random rotation.
   *
  ...
  private @NotNull Solution createNewSolution(@NotNull Vector<Vector2> pointSection) {
    ...
    return new Solution(position, rotationVector, startPoint, this, bound, ratio,
        layerAvailableArea);
  }
  ...
  public void evaluateGeneration() {
    ...
    try {
      ...
      System.out.println(
          "Error : not enough solutions in the population. Please increase POPULATION parameter.");
    }
  }
  ...
  public @NotNull Vector<Solution> reproduce(@NotNull Vector<Solution> solutionsToReproduce) {
    ...
    for (int i = 0; i < numberOfChilds; i++) {
      ...
      Vector2 childAngle = (parent1.getBitAngle()
          .add(parent2.getBitAngle())).mul(0.5)
          .normal();
  ...
  /**
   * Mutate a few solutions to assure diversity. The probability for a Solution to mutate is given
   * by <code>probMutation</code>
   *
  ...
  /**
   * Completes the current generation with new solutions to get exactly <code>popSize</code>
   * solutions.
   *
