...
public class Evolution {
   ...
   *                           score.
   */
  public Evolution(Area layerAvailableArea, Vector<Vector2> pointSection, Vector2 startPoint,
      Vector<Vector2> bound, int nbOfGenerations, int popSize, int RATIO) {
  ...
  public void run() {
    ...
    while (currentGenerationNumber < NB_OF_GENERATIONS) {
      ...
      Vector<Solution> mutatedSolutionsFromLastGen = currentGeneration.mutate(
          generations.get(currentGenerationNumber - 1).solutions);
      Vector<Solution> sortedSolutions = currentGeneration.select(mutatedSolutionsFromLastGen);
      Vector<Solution> reproducedSolutions = currentGeneration.reproduce(
          mutatedSolutionsFromLastGen);
      ...
      Vector<Solution> solutionsToCompleteWith = currentGeneration.completeWithNewSolutions(
          pointSection);
