...
public final class DataLogger {
  ...
  static public long getNumberOfEntries() throws IOException {
    return Files.lines(Paths.get(AI_Tool.DATA_LOG_FILE_PATH))
        .count();
  }
  ...
  static public void saveAllEntries(@NotNull Vector<DataLogEntry> dataSetEntries)
      throws IOException {
  ...
  static public @NotNull DataLogEntry getEntryFromFile(long lineNumber) throws IOException {
    ...
    Optional<String> str = lines.skip(lineNumber - 1)
        .findFirst();
    String line = null;
    if (str.isPresent()) {
      line = str.get();
