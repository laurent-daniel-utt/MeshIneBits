...
/**
 * Generates a .csv dataset format of a line : edge abscissa, bit's angle, point 0 x, point 0 y,
 * point 1 x, point 1 y,...
 */
public final class DataSetGenerator {
  ...
  public static void generateCsvFile() throws IOException {
    ...
    for (int logLine = 1; logLine <= dataLogSize; logLine++) {
      ...
      // format data for dl
      Vector2 startPoint = entry.getAssociatedPoints()
          .firstElement();

      Vector<Vector2> transformedPoints = GeneralTools.getSectionInLocalCoordinateSystem(
          entry.getAssociatedPoints());
      Vector<Vector2> pointsForDl = GeneralTools.getInputPointsForDL(transformedPoints);

      double edgeAbscissa = getBitEdgeAbscissa(entry.getBitPosition(), entry.getBitOrientation(),
          startPoint);
  ...
  /**
   * This method convert a bit position (x, y) to another expression of position (called
   * edgeAbscissa) related to a startPoint and an abscissa related to one bit's vertex. This is
   * helpful for automatic placement on slice's borders,because it guarantees that the bit covers
   * the startPoint, as bit's position is related to it.
   *
   * @param bitOrigin  usual (x, y) coordinates of bit's center
   * @param bitAngle   bit's angle as a vector
   * @param startPoint the startPoint on which the new bit's end should be placed : the intersection
   *                   between the slice border and the last placed bit's end edge
   * @return the edgeAbscissa
   */
  private static double getBitEdgeAbscissa(@NotNull Vector2 bitOrigin, @NotNull Vector2 bitAngle,
      Vector2 startPoint) {
   ...
   * between -90 and 90. Otherwise, as the orientation is expressed in regards to the center of the
   * bit, the angles -100 and -10 degrees (for example) would have been equivalent.
   */
  private static double getBitAngleInLocalSystem(@NotNull Vector2 bitAngle,
      @NotNull Vector<Vector2> sectionPoints) {
    Vector2 localCoordinateSystemAngle = Vector2.getEquivalentVector(
        GeneralTools.getLocalCoordinateSystemAngle(sectionPoints));

    System.out.println(
        "localCoordinateSystemAngle = " + localCoordinateSystemAngle.getEquivalentAngle2());
