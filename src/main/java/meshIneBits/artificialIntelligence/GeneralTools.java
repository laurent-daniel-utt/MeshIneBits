...
/**
 * GeneralTools groups together the methods used to prepare the data for Neural Network or Genetic
 * Algorithms. It also provides methods to perform intersection point search.
 */
public class GeneralTools {

  /**
   * Returns the points of each bound of a given Slice The points are rearranged to be in the
   * correct order.
   *
  ...
  public @NotNull Vector<Vector<Vector2>> getBoundsAndRearrange(@NotNull Slice currentSlice) {
    ...
    Vector<Vector<Segment2D>> borderList = rearrangeSegments(
        (Vector<Segment2D>) currentSlice.getSegmentList()
            .clone());
   ...
   * @return the rearranged segments. Returns more than one Vector of Segment2D if there's more than
   * one bound on the Slice
   */
  private static @NotNull Vector<Vector<Segment2D>> rearrangeSegments(
      @NotNull Vector<Segment2D> segmentList) {
  ...
  /**
   * Searches the next segment of the given segment, in a list of segments. And returns the
   * rearranged list.
   *
   ...
   * @see #rearrangeSegments
   */
  private static @NotNull Vector<Segment2D> searchNextSegment(@NotNull Segment2D segment,
      @NotNull Vector<Segment2D> segmentList, @NotNull Vector<Segment2D> newSegmentList) {
  ...
  /**
   * Calculates the angle of the local coordinate system used for the coordinate system
   * transformation made by {@link #getSectionInLocalCoordinateSystem(Vector)}.
   *
  ...
  /**
   * Translates and rotates a curve to put it in a local coordinate system, centered on the first
   * point of the curve, with the abscissa axis in the direction of the average angle of the curve
   * (regarding in the global coordinate system).
   *
   * @param sectionPoints the points we want to convert in a local coordinate system
   * @return a new {@link Vector} that is the points entered as parameter, transformed in the local
   * coordinate system.
   */
  public static @NotNull Vector<Vector2> getSectionInLocalCoordinateSystem(
      @NotNull Vector<Vector2> sectionPoints) {
    /*
     * TODO: 2021-01-20
   ...
   */
  private static @NotNull Vector<Vector2> transformCoordinateSystem(@NotNull Vector<Vector2> points,
      double angle) {
    angle = Math.toRadians(angle);
    Vector<Vector2> finalPoints = new Vector<>();
    finalPoints.add(new Vector2(0, 0)); // first point is always on origin
    double translatX =
        points.firstElement().x * Math.cos(angle) + points.firstElement().y * Math.sin(angle);
    double translatY =
        -points.firstElement().x * Math.sin(angle) + points.firstElement().y * Math.cos(angle);
    for (int i = 1; i < points.size(); i++) {
      double x = points.get(i).x * Math.cos(angle) + points.get(i).y * Math.sin(angle) - translatX;
      double y =
          -points.get(i).x * Math.sin(angle) + points.get(i).y * Math.cos(angle) + -translatY;
   ...
   * @param startPoint the point on which the left side of the bit will be placed. startPoint must
   *                   be on the polygon.
   * @return a vector of vector2, the part of the polygon which can be used to place a bit
   */
  public static @NotNull Vector<Vector2> getSectionPointsFromBound(
      @NotNull Vector<Vector2> polyPoints, Vector2 startPoint) {
    ...
    sectionPoints.add(
        startPoint); // first we add the startPoint which is the first point of the section
  ...
  /**
   * Checks if most of the points given as parameter are located at the left of a reference point,
   ...
   */
  public static boolean arePointsMostlyOrientedToTheLeft(@NotNull Vector<Vector2> points,
      @NotNull Vector2 refPoint) {
    int leftPoints = 0;
    int rightPoints = 0;
  ...
  /**
   * Positions a precise number of points on a section of points. Each point is equally spaced to
   * the next point.
   *
   * @param sectionPoints the initial section of points.
   * @return a new section of points, composed of the equally spaced points.
   */
  public static @NotNull Vector<Vector2> getInputPointsForDL(
      @NotNull Vector<Vector2> sectionPoints) {
    int nbPoints = 30;
   ...
   */
  private static @NotNull Vector<Vector2> repopulateWithNewPoints(int nbNewPoints,
      @NotNull Vector<Vector2> points) {
    ...
    for (int i = 0; i < nbNewPoints; i++) { // Placer un nouveau point
      ...
      while (basePointsIndex < points.size() - 2
          && baseSegmentSum + segmentLength.get(basePointsIndex) <= newSegmentSum) {
      ...
      if (points.get(basePointsIndex).x == points.get(basePointsIndex + 1).x
          && points.get(basePointsIndex).y <= points.get(
          basePointsIndex + 1).y) { // then the segment is vertical
        segmentAngle = Math.PI / 2;
      } else {
        segmentAngle = Math.atan((points.get(basePointsIndex + 1).y - points.get(basePointsIndex).y)
            / (points.get(basePointsIndex + 1).x - points.get(
            basePointsIndex).x)); // slope of the segment
      }
      ...
      absNewPoint =
          points.get(basePointsIndex).x + sign * (newSegmentSum - baseSegmentSum) * Math.cos(
              segmentAngle);
      ordNewPoint =
          points.get(basePointsIndex).y + sign * (newSegmentSum - baseSegmentSum) * Math.sin(
              segmentAngle);
   ...
   * This an alternative approach to {@link #getInputPointsForDL(Vector)} method : the returned
   * values describe approximately the shape of the section of points entered as parameter. Then the
   * coefficients returned could be used in a neural net. The interest of this method over {@link
   * #getInputPointsForDL(Vector)} is that it reduces the number of features injected in the neural.
   * In return, the representation of the section may be less accurate. The steps below describe how
   * the double non-linear regression works : 1 - The section is split into two curves : x(t) and
   * y(t), where the parameter t is the curvilinear abscissa of the section. 2 - A non-linear
   * regression is made over each curve. 3 - We get two arrays of coefficients related to the two
   * curves.
   *
   * @param sectionPoints the points on which the regression is performed.
   * @param degree        the degree of the regression, witch is linked to the accuracy of the
   *                      regression performed.
   * @return a {@link Vector} of two {@link Vector} of coefficients related the section entered as
   * parameter.
   */
  @SuppressWarnings("unused")
  private static @NotNull Vector<Vector<Double>> getInputSlopesForDL(Vector<Vector2> sectionPoints,
      int degree) {
  ...
  /**
   * Return the next Bit2D start point. It is the intersection between the slice and the end side of
   * the Bit2D.
   *
   ...
   */
  public Vector2 getNextBitStartPoint(@NotNull Bit2D bit, @NotNull Vector<Vector2> boundPoints)
      throws Exception {
  ...
  /**
   * Returns points all points associated with a Bit2D. Points associated are the points of the
   * Slice from the startPoint of the Bit2D, till the distance with the point become greater than
   * the lengh of a Bit2D.
   *
   * @param bit2D The Bit2D we want to get the points associated with.
   * @return the associated points.
   */
  public @NotNull Vector<Vector2> getCurrentLayerBitAssociatedPoints(@NotNull Bit2D bit2D)
      throws Exception {
   ...
   */
  public static @Nullable Vector2 getBitAndContourFirstIntersectionPoint(@NotNull Bit2D bit,
      @NotNull Vector<Vector2> boundPoints) {
    ...
    while (!scanCompleted) { //look for an intersecion
      ...
      for (Segment2D bitSide : bitSides) {
        Vector2 intersectionPoint = Segment2D.getIntersectionPoint(bitSide,
            boundSegments.get(iSeg));
  ...
  /**
   * Looks for an intersection point between a side of a bit and a closed contour (bound). TJe point
   * returned is the first intersecton between the bit and the contour, while scanning the contour
   * in the direction of increasing indices of the points of the contour.
   *
   ...
   */
  public static Vector2 getBitAndContourSecondIntersectionPoint(@NotNull Bit2D bit,
      @NotNull Vector<Vector2> boundPoints) {
  ...
  /**
   * Converts a list of {@link Vector2} to a list of {@link Vector} that would connect each point to
   * the other, following the order of the list of points given as entry.
   *
  ...
  /**
   * Find an approximation of the intersection point between a segment and a circle If there is more
   * than one intersection, this method will return the point that is the closest to the end of the
   * segment. Initial condition : an intersection should exist
   *
   ...
   */
  public static @NotNull Vector2 circleAndSegmentIntersection(Vector2 center, double radius,
      @NotNull Segment2D seg) {
  ...
  public static boolean isPointOnSegment(Vector2 v, @NotNull Segment2D s) {
    ...
    return Math.abs(Vector2.dist(s.start, v) + Vector2.dist(s.end, v) - s.getLength())
        < errorAccepted;
  }
}