...
public class COAlgo2 {
    ...
    private IntVar[] yLigne;
    private IntVar[] xLigne;
    ...
    public Vector<Bit2D> getBits(Slice slice, double minWidth, double numberMaxBits) throws NoninvertibleTransformException {
        ...
        for (Vector<Vector2> bound : bounds) {
            ...
            do {
                ...
                Bit2D bit = getBitFromSectionWithCP(sectionPoints, bound);
                bits.add(bit);
...
//                bits.add(placement.bit2D);
            ...
            } while (!((listContainsAsGoodAsEqual(veryFirstStartPoint, sectionPoints) && iBit > 1)
                    || listContainsAllAsGoodAsEqual(bound, sectionPoints)) && iBit < 30);//todo max bits
    ...
    private Bit2D getBitFromSectionWithCP(Vector<Vector2> sectionPoints, Vector<Vector2> bound) throws NoninvertibleTransformException {
...
//###############################################################################################################
        ...
        System.out.println("BEST SOLVED : Sum=" + solution.getIntVal(model.getObjective()
                                                                             .asIntVar()));
...
//###############################################################################################################
        ...
        for (int i = 0; i < NBPOINTS; i++) {
            lignePointsLocal.add(
                    new Vector2(solution.getIntVal(xLigne[i]), solution.getIntVal(yLigne[i]))
            );
        }
        ...
        Bit2D bit = getBit(lignePointsGlobal);
    ...
    private Bit2D getBit(Vector<Vector2> lignePoints) {
        ...
        Vector2 firstPoint = lignePoints.firstElement();
        Vector2 lastPoint = lignePoints.lastElement();
        //todo enlever ? on recalcule angleLocalSystem
        Vector2 angleVector = new Vector2(lastPoint.x - firstPoint.x, lastPoint.y - firstPoint.y).normal();

//        Vector2 center = new Vector2(
//                firstPoint.x+ CraftConfig.bitWidth/2*Math.cos(Math.toRadians(angleLocalSystem)),
//                firstPoint.y+ CraftConfig.lengthFull/2*Math.sin(Math.toRadians(angleLocalSystem))
//        );
...
//                .add(orthogonal.mul(CraftConfig.bitWidth/2));
        ...
        DebugTools.pointsToDrawBLUE.add(bit.getBitSidesSegments()
                                                .get(0).start);
        DebugTools.pointsToDrawBLUE.add(bit.getBitSidesSegments()
                                                .get(1).start);
        DebugTools.pointsToDrawBLUE.add(bit.getBitSidesSegments()
...
//on définit les variables et les contraintes du modele
        ...
        xLigne = new IntVar[NBPOINTS];
        yLigne = new IntVar[NBPOINTS];
        int L = NBPOINTS - 1;
        xLigne[0] = model.intVar("x_" + 0, (int) startPoint.x - OFFSET, (int) startPoint.x + OFFSET);
        yLigne[0] = model.intVar("y_" + 0, (int) startPoint.y - OFFSET, (int) startPoint.y + OFFSET);
        xLigne[L] = model.intVar("x_" + L, (int) endPoint.x - OFFSET, (int) endPoint.x + OFFSET);
        yLigne[L] = model.intVar("y_" + L, (int) endPoint.y - OFFSET, (int) endPoint.y + OFFSET);


//      FORMULE xLigne[i] = xLigne[0] + (xLigne[-1]-xLigne[0])/i
        for (int i = 1; i < NBPOINTS - 1; i++) {
            xLigne[i] = model.intVar("x_" + i, xLigne[L].sub(xLigne[0])
                    .div(NBPOINTS - 1)
                    .mul(i)
                    .add(xLigne[0])
                    .intVar());
            yLigne[i] = model.intVar("y_" + i, yLigne[L].sub(yLigne[0])
                    .div(NBPOINTS - 1)
                    .mul(i)
                    .add(yLigne[0])
                    .intVar());
            xLigne[i].eq(xLigne[L].sub(xLigne[0])
                                 .div(NBPOINTS - 1)
                                 .mul(i)
                                 .add(xLigne[0]))
                    .post();
            yLigne[i].eq(yLigne[L].sub(yLigne[0])
                                 .div(NBPOINTS - 1)
                                 .mul(i)
                                 .add(yLigne[0]))
                    .post();
...
//           model.arithm(xLigne[i+1].sub(xLigne[i]).intVar(),"=",xSection[i+1].sub(xSection[i]).intVar()).post();
        ...
        for (int i = 0; i < NBPOINTS; i++) {
            IntVar distX = model.intVar("D" + i + "xLigne", xLigne[i].sub(xSection[i])
                    .intVar());
            IntVar distY = model.intVar("D" + i + "yLigne", yLigne[i].sub(ySection[i])
                    .intVar());
            distX.eq(xLigne[i].sub(xSection[i])
                             .intVar())
                    .post();
            distY.eq(yLigne[i].sub(ySection[i])
                             .intVar())
                    .post();
...
//            model.arithm(xLigne[i],"<=",xSection[i]).post();
            model.arithm(yLigne[i], "<", ySection[i])
                    .post();

            //on crée des variables intermédiaires des distancesX et Y qui sont les valeurs absolues
            IntVar distXAbs = model.intVar("D" + i + "xAbs", distX.abs()
                    .intVar());
            IntVar distYAbs = model.intVar("D" + i + "yAbs", distY.abs()
                    .intVar());
            distXAbs.eq(distX.abs()
                                .intVar())
                    .post();
            distYAbs.eq(distY.abs()
                                .intVar())
                    .post();

            distances[i] = model.intVar("D" + i, distXAbs.add(distYAbs)
                    .intVar()); //OK
            distances[i].eq(distXAbs.add(distYAbs)
                                    .intVar())
                    .post();//OK attention on n'a pas les carrés et la racine
        }

        //variable somme des distances
        IntVar SUM = model.intVar("SUM", 0, 600);
        //sommer les valeurs absolues des distances
        model.sum(distances, "=", SUM)
                .post();
