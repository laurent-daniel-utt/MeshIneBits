...
import meshIneBits.util.supportExportFile.ProjectXMLTool;
...
public class Project extends Observable implements Observer, Serializable {
  ...
  /**
   * Set the new project to ready
   */
  public Project() {
    setState(ProjectEvents.READY);
  ...
  public static Project open(File file) throws IOException, ClassNotFoundException {
  ...
  public void importModel(String filepath) throws Exception {
    ...
    setState(ProjectEvents.IMPORTING);
    ...
    try {
      ...
      e.printStackTrace();
      setState(ProjectEvents.IMPORT_FAILED);
      return;
    }
    ...
    // Signal to update
    setState(ProjectEvents.IMPORTED);
  }
public ArrayList<ArrayList<Strip>> getStripes(){
  ...
  public void slice() throws Exception {
    ...
    setState(ProjectEvents.SLICING);
    ...
    slicer.sliceModel();
    // ProjectEvents.SLICED will be sent in update() after receiving
    // signal from slicer
  }

  /**
   * Given a certain template, pave the whole project sequentially
   *
   * @param template an automatic builder
   * @throws Exception when an other action is currently executing or {@link Project} is not sliced
   *                   yet
   */
  public void pave(PatternTemplate template) throws Exception {
    getLayers().forEach(layer -> layer.getRemovedSubBitsPositions().clear());
    pavementSafetyCheck();
    setState(ProjectEvents.PAVING_MESH);
    // ProjectEvents.PAVED_MESH will be sent in update() after receiving
...
//            (new Thread(pavingWorkerMaster)).start();
  ...
  private void pavementSafetyCheck() throws Exception {
    ...
    if (!isSliced()) {Logger.updateStatus("make sure the Project is Sliced");
      throw new Exception("The project cannot be paved until it is sliced");

    }
  }

  public boolean isSliced() {
    return state.getCode() >= ProjectEvents.SLICED.getCode();
  }
  ...
  public void optimize() throws Exception {
    ...
    setState(ProjectEvents.OPTIMIZING_MESH);
    // ProjectEvents.OPTIMIZED_MESH will be sent in update() after receiving
    // enough signals from layers
    ProjectOptimizerMaster projectOptimizerMaster = new ProjectOptimizerMaster();
    projectOptimizerMaster.addObserver(this);
    Thread t = new Thread(projectOptimizerMaster);
    t.start();
  }
  ...
  private void optimizationSafetyCheck() throws Exception {
    ...
    if (!isPaved()) {
      throw new Exception("The project cannot be auto optimized until it is fully paved.");
    }
  }
  ...
  public  boolean isPaved() {
    if (state.getCode() >= ProjectEvents.PAVED_MESH.getCode()) {
      return true;
    } else {
      if (layers.isEmpty()) {
        return false;
      ...
      if (layers.stream()
        ...
        state = ProjectEvents.PAVED_MESH;
        return true;
      } else {
        return false;
      }
  ...
  public void optimize(Layer layer) throws Exception {
    ...
    setState(ProjectEvents.OPTIMIZING_LAYER);
    LayerOptimizer layerOptimizer = new LayerOptimizer(layer);
    layerOptimizer.addObserver(this);
    // ProjectEvents.OPTIMIZED_LAYER will be sent after completed the task
   ...
   */
  public void glue() {
    setState(ProjectEvents.GLUING);
    // TODO run glue inserting in each layer
    // ProjectEvents.GLUED will be sent in update() after receiving
    // enough signals from layers
  }
  ...
  public void export(File file) throws Exception {
    ...
    setState(ProjectEvents.EXPORTING);
    ProjectXMLExporter projectXMLExporter = new ProjectXMLExporter(file);
//        Thread t = new Thread(projectXMLExporter);
//        t.start();
    projectXMLExporter.run();
    setState(ProjectEvents.EXPORTED);
  }
  ...
  private void exportationSafetyCheck() throws Exception {
    ...
    if (!isPaved()) {
      throw new Exception("Project in unpaved");
    }
  }
    ...
    if (arg instanceof ProjectEvents) {
      switch ((ProjectEvents) arg) {
        ...
        case SLICED:
          ...
          setSkirtRadius();
          initLayers();
          setState(ProjectEvents.SLICED);
          break;
        case PAVING_MESH:
          break;
        case PAVED_MESH:
          setState(ProjectEvents.PAVED_MESH);
          break;
        case PAVING_LAYER:
          break;
        case PAVED_LAYER:
          Logger.updateStatus("Layer paved");
          setState(ProjectEvents.PAVED_LAYER);
          break;
        case OPTIMIZING_LAYER:
          break;
        case OPTIMIZED_LAYER:
          setState(ProjectEvents.OPTIMIZED_LAYER);
          break;
        case OPTIMIZING_MESH:
          break;
        case OPTIMIZED_MESH:
          setState(ProjectEvents.OPTIMIZED_MESH);
          break;
        case GLUING:
          break;
        case GLUED:
        ...
        case EXPORTED:
          ...
          break;
        case SCHEDULING:
          setState(ProjectEvents.SCHEDULING);
          break;
        case SCHEDULED:
          Logger.updateStatus("Project scheduled");
          setState(ProjectEvents.SCHEDULED);
          break;
      }
    }
  }
  ...
  public void runScheduler() throws Exception {
    ...
    Logger.updateStatus("Scheduling project");
    // Scheduler will send a signal ProjectEvents.SCHEDULED to Project.update()
//        (new Thread(scheduler)).start();
    scheduler.run();
  }

  public ProjectEvents getState() {
    return state;
  }

  /**
   * Update the current state of project and notify observers with that state
   *
   * @param state a value from predefined list
   */
  public void setState(ProjectEvents state) {
    this.state = state;
    setChanged();
  ...
  public void pave(PatternTemplate patternTemplate, Layer layer) throws Exception {
    ...
    setState(ProjectEvents.PAVING_LAYER);
    // ProjectEvents.PAVED_LAYER will be sent to update()
...
//        (new Thread(regionPaver)).start();
  ...
  public void saveAs(File file) throws IOException {
    ...
    try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(file))) {
      ...
      oos.flush();
      setChanged();
      notifyObservers(ProjectEvents.SAVED);
    } catch (IOException e) {
      setChanged();
      notifyObservers(ProjectEvents.SAVE_FAILED);
      throw e;
    }
  }
  ...
  private class PavingWorkerMaster extends Observable implements Observer, Runnable {
    ...
    @Override
    public void run() {
      Logger.updateStatus("Paving project parallelly with " + originalPatternTemplate.getCommonName());
  ...
  /**
   * Managing optimization of a layer, then reporting to {@link ProjectOptimizerMaster} or {@link
   * Project}
   */
  private class LayerOptimizer extends Observable implements Runnable {
    ...
    public void run() {
      ...
      setChanged();
      notifyObservers(ProjectEvents.OPTIMIZED_LAYER);
    }
  }

  /**
   * Managing process of optimizing consequently all layers, then reporting to {@link Project}
   */
  private class ProjectOptimizerMaster extends Observable implements Runnable, Observer {
    ...
    private List<ProjectOptimizerSlave> slaves = new ArrayList<>();

    ProjectOptimizerMaster() {
      for (Layer layer : layers) {
        ProjectOptimizerSlave slave = new ProjectOptimizerSlave(layer);
    ...
    @Override
    public void run() {
      slaves.forEach(projectOptimizerSlave -> (new Thread(projectOptimizerSlave)).start());
    }

    @Override
    public synchronized void update(Observable o, Object arg) {
      if (o instanceof ProjectOptimizerSlave) {
        finishedJob++;
        Logger.setProgress(finishedJob, layers.size());
        int ir = (int) arg;
        Layer layer = ((ProjectOptimizerSlave) o).getLayer();
        if (ir > 0) {
      ...
      if (finishedJob == layers.size()) {
        ...
        setChanged();
        notifyObservers(ProjectEvents.OPTIMIZED_MESH);
      }
    }
  }

  private class ProjectOptimizerSlave extends Observable implements Runnable {

    private Layer layer;

    ProjectOptimizerSlave(Layer layer) {
      this.layer = layer;
    }
  ...
  private class ProjectXMLExporter extends Observable implements Runnable {

    private final File file;

    ProjectXMLExporter(File file) {
      this.file = file;
    }

    @Override
    public void run() {
//            XmlTool2 xt = new XmlTool2(Project.this, file.toPath());
//            xt.writeXmlCode();
      ProjectXMLTool xt = new ProjectXMLTool(file.toPath());
      xt.writeMeshToXML(Project.this);
      setChanged();
      notifyObservers(ProjectEvents.EXPORTED);
    }
  }
  ...
  private class LayerPaver extends Observable implements Runnable {
    ...
    LayerPaver(Layer layer, PatternTemplate patternTemplate) {
      this.layer = layer;
      this.patternTemplate = patternTemplate;
      patternTemplate.ready(Project.this);
    }
  ...
  private class SchedulerNotDefinedException extends Exception {
    ...
    SchedulerNotDefinedException() {
      super("The scheduling method is not defined in the project object");
    }
  }
