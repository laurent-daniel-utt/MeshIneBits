...
public class Optimizer extends Observable implements Observer, Runnable {
    ...
    private Thread threadForAutoOptimizingMesh = null;
    /**
     * What we will try to solve. Either the mesh or the layer.
     */
     ...
     * {@link Mesh}
     *
     * @param layers disintegration of mesh
     */
     ...
     * @deprecated
     */
    public void automaticallyOptimizeMesh(Mesh mesh) {
        if (threadForAutoOptimizingMesh == null || !threadForAutoOptimizingMesh.isAlive()) {
            target = mesh;
            threadForAutoOptimizingMesh = new Thread(this);
            threadForAutoOptimizingMesh.start();
        }
    }
     ...
     *
     * @param o   the layer which has been changed
