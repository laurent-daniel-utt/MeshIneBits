...
public class MeshExport extends PGraphics {
  ...
  private void generateTexture() {
    ...
    for (int i = 0; i < faceCount; ++i) {
      ...
      fc = faceColors[i];
      if (f.length > 4) {
        showWarning("Faces with more than 4 sides cannot be exported with color");
      } else if (f.length == 3) {
        //draw triangle
        textureG.strokeWeight(3);
        if (upper) {
          ...
          writeTexCoord((gX + currX + 1f + .5f) / texture.width,
              (1.0f - (gY + currY + 1f + .5f) / texture.height));
          c = fc[1];
          textureG.fill(c);
          textureG.stroke(c);
          textureG.vertex(currX + 1 + TRIANGLE_RES + .5f, currY + 1 + .5f);
          writeTexCoord((gX + currX + 1f + TRIANGLE_RES + .5f) / texture.width,
              (1.0f - (gY + currY + 1f + .5f) / texture.height));
          c = fc[2];
          textureG.fill(c);
          textureG.stroke(c);
          ...
          writeTexCoord((gX + currX + 1f + .5f) / texture.width,
              (1.0f - (gY + currY + TRIANGLE_RES + 1f + .5f) / texture.height));
          ...
          writeTexCoord((gX + currX + 3f + TRIANGLE_RES + .5f) / texture.width,
              (1.0f - (gY + currY + 3f + .5f) / texture.height));
          c = fc[1];
          textureG.fill(c);
          textureG.stroke(c);
          textureG.vertex(currX + TRIANGLE_RES + 3 + .5f, currY + TRIANGLE_RES + 3 + .5f);
          writeTexCoord((gX + currX + TRIANGLE_RES + 3f + .5f) / texture.width,
              (1.0f - (gY + currY + TRIANGLE_RES + 3f + .5f) / texture.height));
          c = fc[2];
          textureG.fill(c);
          textureG.stroke(c);
          textureG.vertex(currX + 3 + .5f, currY + TRIANGLE_RES + 3 + .5f);
          writeTexCoord((gX + currX + 3f + .5f) / texture.width,
              (1.0f - (gY + currY + TRIANGLE_RES + 3f + .5f) / texture.height));
          textureG.endShape(CLOSE);
          currX += RECT_RES;
        }
        ...
        writeTexCoord((gX + currX + TRIANGLE_RES + 1f + .5f) / texture.width,
            (1.0f - (gY + currY + TRIANGLE_RES + 1f + .5f) / texture.height));
        c = fc[3];
        textureG.fill(c);
        textureG.stroke(c);
  ...
  public void vertex(float x, float y, float z) {
    ...
    if (!ptMap.containsKey(x + "_" + y + "_" + z)) {
      ...
      vertices[vertexCount] = ptMap.get(x + "_" + y + "_" + z)
          .intValue();
    }
    //color
    if (colorFlag) {
  ...
  /**
   * Two dimensional rotation. Same as rotateZ (this is identical to a 3D rotation along the z-axis)
   * but included for clarity -- it'd be weird for people drawing 2D graphics to be using rotateZ.
  ...
  /**
   * Rotate around an arbitrary vector, similar to glRotate(), except that it takes radians (instead
   * of degrees).
   */
  @Override
