...
public static Reconstitute getInstance(){
     ...
     *
     * @param path
     * @return list of subpaths (method used to decompse falltype paths to sub paths)
     */
    ...
    private Area calculateFallenArea(Area remainingArea,Path2D.Double subpath,boolean opp){
        ...
        for(Area area:areas){
            /** l'area qui tombe est celui qui n'est pas tenu par le robot de découpe et donc qui ne contient aucun point
             ...
             d'autre valeur tant qu'on dépasse pas la Holding zone
             */
...
private boolean areaAddedtoFallen(ArrayList<Vector2> points,boolean opp){
   ...
   /* System.out.println("poitillé:"+points.get(1).x+" first:"+points.get(0).x+" last:"+points.get(points.size()-1).x);
    System.out.println("poitillé:"+points.get(1).x+" milieu"+(points.get(0).x+points.get(points.size()-1).x)/2);
    if(points.get(1).x>(points.get(0).x+points.get(points.size()-1).x)/2) {System.out.println("added to fallen");
            return true;}*/
   ...
   /* if(points.get(1).y>(points.get(0).y+points.get(points.size()-1).y)/2 && TopCornerCutting(points.get(0),points.get(points.size()-1))) {System.out.println("added to fallen1");
        return true;}
    else if (points.get(1).y<(points.get(0).y+points.get(points.size()-1).y)/2 && BottomCornerCutting(points.get(0),points.get(points.size()-1))) {System.out.println("added to fallen2");
        return true;
...
//FIXME x=80
  private boolean checkpointOnExtremity(Vector2 point){
     ...
     * @return true if on pposite sides, false if not
     */
