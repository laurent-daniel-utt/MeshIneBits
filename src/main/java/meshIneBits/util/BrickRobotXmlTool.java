...
/**
 * A try to generate an XML file compatible with the BrickRobot machine, but it doesn't work because
 * we can't handle complex path to position the bits
 */
class BrickRobotXmlTool {
  ...
  public BrickRobotXmlTool(Project part, Path fileLocation) {
    this.part = part;
  ...
  private void setFileToXml() {
    ...
    filePath = Paths.get(filePath.getParent()
        .toString() + "\\" + fileName);
  }
  ...
  private void writeLayer(Layer layer) {
    double zPlateau =
        layer.getLayerNumber() * CraftConfig.bitThickness - (CraftConfig.bitThickness / 2);
    writer.println("	<PLATEAU z=\"" + zPlateau + "\"/>");
    for (int i = 0; i < layer.getBits3dKeys()
        .size(); i++) {
      writeBit(layer.getBit3D(layer.getBits3dKeys()
          .get(i)), zPlateau);
    }
  }
  ...
  private void writeBit(Bit3D bit, double zPlateau) {
    ...
    writer.println("		<PATH config_data=\"N, ,0,0\" p=\"0.0\" r=\"" + (
        bit.getOrientation()
            .getEquivalentAngle() + 90) + "\" w=\"0.0\""
        + " x=\"" + bit.getOrigin().x + "\""
        + " y=\"" + bit.getOrigin().y + "\""
        + " z=\"" + (zPlateau + CraftConfig.bitThickness + SAFETY_MARGIN) + "\"/>");
    writer.println("		<POSI config_data=\"N, ,0,0\" p=\"0.0\" r=\"" + (
        bit.getOrientation()
            .getEquivalentAngle() + 90) + "\" w=\"0.0\""
        + " x=\"" + bit.getOrigin().x + "\""
        + " y=\"" + bit.getOrigin().y + "\""
