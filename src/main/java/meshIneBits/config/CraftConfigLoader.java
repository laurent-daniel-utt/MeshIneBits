...
//                        System.out.println("Value: " + field.getName() + ":" + val);
   ...
   *
   * @param filePath if <tt>null</tt>, program will load default file from user's home directory
   * @return <tt>null</tt> when loading fails.
   */
  public static PatternConfig loadPatternConfig(File filePath) {
    if (filePath == null) {
      filePath = new File(
          System.getProperty("user.home") + File.separator + ".MeshIneBits.PatternConfig."
              + PATTERN_CONFIG_EXTENSION);
    }
