...
public class UPPScheduleProject extends UtilityParametersPanel {

  public UPPScheduleProject(ProjectController projectController) {
    ...
    processingViewSchedulerBtn.addActionListener(e -> {
      try {
        projectController.scheduleMesh();
      } catch (Exception e1) {
    ...
    schedulerChoice.addActionListener(e -> {
      ...
      try {
        projectController.setScheduler((AScheduler) sC.getSelectedItem());
      } catch (Exception e1) {
        projectController.handleException(e1);
      }
    });
