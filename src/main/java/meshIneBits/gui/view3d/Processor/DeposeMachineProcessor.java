...
public class DeposeMachineProcessor implements UIPWListener {
  ...
  public DeposeMachineProcessor(DepositingProcessCallback callback) {
    ...
    subscriptionTask=new DeposeMachineProcessor.SubscriptionTask();
 ...
 //   MultiThreadServiceExecutor.instance.execute(subscriptionTask);
  ...
  public void onActionListener(Object callbackObj, String event, Object value) {
    switch (event) {
      case (GraphicElementLabel.START_DEPOSITING_MACHINE):
        startDepose();
        break;
      case (GraphicElementLabel.STOP_DEPOSITING_MACHINE):
        stopDepose();
        break;
      case (GraphicElementLabel.PAUSE_DEPOSITING_MACHINE):
        pauseDepose();
        break;
      case (GraphicElementLabel.CONTINUE_DEPOSITING_MACHINE):
        continueDepose();
        break;
      case (GraphicElementLabel.RESET_DEPOSITING_MACHINE):
        resetDepose();
        break;
      case (GraphicElementLabel.CONTINUE_AFTER_E_STOP_DEPOSITING_MACHINE):
        continueAfterESTOP();
        break;
      case (GraphicElementLabel.CONTINUE_AFTER_TURN_OFF_DEPOSITING_MACHINE):
        continueAfterTurnOff();
        break;
      case (GraphicElementLabel.CAMERA_LOGIN_DEPOSITING_MACHINE):
        cameraLogin();
        break;
      case (GraphicElementLabel.CAMERA_CAPTURE_IMAGE_DEPOSITING_MACHINE):
        cameraCaptureImage();
        break;
      case (GraphicElementLabel.ACKNOWLEDGE_ERROR_DEPOSITING_MACHINE):
        acknowledgeError();
        break;
    }
  }
