  ...
  public void settings() {
    PJOGL.setIcon(
        Objects.requireNonNull(this.getClass()
                .getClassLoader()
                .getResource("resources/icon.png"))
            .getPath());
...
//            builder.buildBits(shapeMap);
  ...
  private void drawBits(Vector<Pair<Position, PShape>> shapeMap, int n) {
    ...
    for (int i = 0; i < shapeMap.size(); i++) {
      if (j < n) {
        pushMatrix();
        float[] t = shapeMap.get(i)
            .getKey()
            .getTranslation();
        translate(t[0], t[1], t[2]);
        translate((float) controllerView3D.getCurrentMesh()
                .getModel()
                .getPos().x,
            (float) controllerView3D.getCurrentMesh()
                .getModel()
                .getPos().y,
            (float) controllerView3D.getCurrentMesh()
                .getModel()
        ...
        rotateZ(radians(shapeMap.get(i)
            .getKey()
            .getRotation()));

        PShape s = shapeMap.get(i)
            .getValue();
