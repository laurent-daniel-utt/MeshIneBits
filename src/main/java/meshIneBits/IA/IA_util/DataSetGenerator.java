 ...
 */
public final class DataSetGenerator { // todo il y a sans doute des classe de DataPreparation qui auraient leur place ds cette classe
    final static String dataSetFilePath = "dataSet.csv";

    /**
     * generates a .cvs file that can be used by a neural network
     * @return
     */
    public static int generateCsvFile(){
        final String fileName = "dataAsPoints.csv";
        ...
        for (int logLine=1; logLine<=dataLogSize; logLine++){
            ...
            //generate one line (corresponds to the data of one bit)
            String csvLine = "";
            csvLine += edgeAbscissa; // add position
            csvLine += "," + orientation; //add orientation
            ...
            FileWriter fw = null;
            try {
                fw = new FileWriter(dataSetFilePath, true);
                fw.write(csvLine + "\n");
                fw.close();
            } catch (IOException e) {
                e.printStackTrace();
