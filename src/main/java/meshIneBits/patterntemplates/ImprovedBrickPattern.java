...
public class ImprovedBrickPattern extends PatternTemplate {
  ...
  public Pavement pave(Layer layer) {
    ...
    double diffRotation = (double) config.get("diffRotation")
        .getCurrentValue();
  ...
  private Collection<Bit2D> pave(int layerNumber, Vector2 patternStart, Vector2 patternEnd) {
    ...
    // Setup parameters
    bitsWidthSpace = (double) config.get("bitsWidthSpace")
        .getCurrentValue();
    bitsLengthSpace = (double) config.get("bitsLengthSpace")
        .getCurrentValue();
    double diffxOffset = (double) config.get("diffxOffset")
        .getCurrentValue();
    double diffyOffset = (double) config.get("diffyOffset")
        .getCurrentValue();
    ...
    while (_1stBit.y - CraftConfig.bitWidth / 2
        + lineNum * (CraftConfig.bitWidth + bitsLengthSpace)
        <= patternEnd.y + CraftConfig.bitWidth / 2) {
      // Horizontally
      if (lineNum % 2 == 0) {
        fillHorizontally(
   ...
   */
  private void fillHorizontally(Vector2 _1stBitOrigin, Vector<Bit2D> bits, Vector2 patternStart,
      Vector2 patternEnd) {
  ...
  public int optimize(Layer actualState) {
    ...
    while (!allFail) {
      ...
      for (Vector2 irBitKey : irregularBitKeys) {
        ...
        localDirectionToMove = attemptToSolve(currentPavement, boundary, irBitKey,
            irregularBitKeys);
      ...
      if (localDirectionToMove != null && irBitKeyToMove != null) {
        ...
        Vector2 newPos = this.pushBit(currentPavement, boundary, irBitKeyToMove,
            localDirectionToMove);
   ...
   * <p>
   * Prioritizing the height's sides. If the obtained state has less irregular bits, we will follow
   * that way. Note: we also try to cover what we left behind by a full bit.
   *
   * @param pavement       the selected pavement in the layer. This method will work on its clone
   ...
   * @return the first direction which reduce the total number of irregular bits in the pavement.
   * <tt>Null</tt> if no way to get better state. Calculated in local coordinate system of input
   * bit
   */
  private Vector2 attemptToSolve(Pavement pavement,
    ...
    for (Vector2 localDirectionToTry : localDirectionsForTrying) {
      ...
      // Re-validate
      if (initialIrregularities > DetectorTool.detectIrregularBits(clonedPavement)
          .size()) {
  ...
  private Vector2 cover(Pavement actualState,
    ...
    if ((movedBit.getLength() == CraftConfig.lengthFull
      ...
      return this.pushBit(actualState, boundary, coveringBitKey,
          localDirectionToMove.getOpposite());
    }
    return null;
  }
   ...
   * <p>
   * Automatically reduce back every bit in front of it. The step of push is determined by
   * direction, either a half of {@link CraftConfig#lengthFull} or {@link CraftConfig#bitWidth}
   *
   * @param actualState          current situation
  ...
  private Vector2 pushBit(Pavement actualState,
    ...
    if (actualState.getBit(newOrigin) == null) {
      return null;
    }
    ...
    for (Vector2 bitKey : bitPartiallyInFrontOfBitToMove) {
      ...
      Bit2D bitToReduce = actualState.getBit(bitKey)
          .clone();
        ...
        coveringCenter = initialCenter.add(
            ...
            .sub(
                localDirectionToPush
                    ...
                    .mul(verticalDisplacement / 2 + additionalVerticalDisplacement)
   ...
   * @param localDirectionToReduce in the coordinate system of bit. Should be either (0, 1), (0,
   *                               -1), (1, 0), (-1, 0).
   * @param lengthToReduce         in millimeter. If greater than sides, the bit will be removed.
   *                               Half of {@link CraftConfig#lengthFull} or {@link
   ...
   * <p>
   * Ensure to use {@link #checkAdjacence(Bit2D, Bit2D) checkAdjacence} before this.
   *
   * @param bit1           the first input bit (reference bit)
   * @param bit2           the second input bit (bit to check)
   * @param localDirection in the local coordinate system of bit1. Either (0, 1), (1, 0), (0, -1),
   *                       (-1, 0)
   * @return <tt>true</tt> if half of sum of 2 bits' sides is greater than
   * distance between center; anh <tt>bit2</tt> faces in <tt>localDirection</tt> of <tt>bit1</tt>.
   */
  ...
  /**
   * Check if the <tt>bit2</tt>'s center is in front of the <tt>bit1</tt>'s side, given the
   * direction.
   * <p>
   * Ensure to use {@link #checkInFront(Bit2D, Bit2D, Vector2) checkInFront} before this.
   *
   * @param bit1           the first input bit (reference bit)
   * @param bit2           the second input bit (bit to check)
