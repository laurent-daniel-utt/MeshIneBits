...
public class UnitSquarePattern extends PatternTemplate {
  ...
  public static final transient java.util.logging.Logger LOGGER = Logger.createSimpleInstanceFor(
      UnitSquarePattern.class);
  ...
  public Pavement pave(Layer layer) {
    ...
    applyQuickRegroup = (boolean) config.get(APPLY_QUICK_REGROUP)
        .getCurrentValue();
    // Limit depth of search
    limitActions = (int) Math.round((double) config.get(LIMIT_ACTIONS)
        .getCurrentValue());
    // Cut off minor details or not
    cutDetails = (boolean) config.get(CUT_DETAILS)
        .getCurrentValue();
    // Get the boundaries
    List<Area> zones = AreaTool.getContinuousSurfacesFrom(layer.getHorizontalSection());
    // Rotation
    double rotation = Math.toRadians(
        ((double) config.get(INCREMENTAL_ROTATION)
    ...
    for (Area zone : zones) {
      ...
      if (matrix.resolve()) {
        ...
        Set<Bit2D> trueBits = biasedBits.stream()
            .map(bit -> bit.createTransformedBit(inv))
  ...
  private void calcUnitSizeAndLimits() {
    this.unitLength =
        ((double) config.get("horizontalMargin")
            .getCurrentValue()) + CraftConfig.suckerDiameter;
    this.unitWidth =
        ((double) config.get("verticalMargin")
            .getCurrentValue()) + CraftConfig.suckerDiameter;
  ...
  /**
   * Describe the relative position of a {@link UnitMatrix.UnitSquare} in respect to predefined
   * area
   *
   * @author Quoc Nhat Han TRAN
   */
  ...
  /**
   * To decide which {@link Puzzle} will be the first target to begin the dfs search
   *
  ...
  /**
   * @param name "NATURAL" or "DUTY_FIRST". If not found, will set NATURAL by default
   * @see Strategy
   */
  ...
  /**
   * @param name "NATURAL" or "BORDER_FIRST". If not found, will set NATURAL by default
   * @see Strategy
   */
  ...
  private interface Puzzle {
    ...
    /**
     * Check if 2 puzzle can be merged. Should check this before {@link #merge(Puzzle)}
     *
     * @param puzzle a {@link UnitMatrix.UnitSquare} or {@link UnitMatrix.Polyomino}
     */
     ...
     * @return A new {@link UnitMatrix.Polyomino} containing all {@link UnitMatrix.UnitSquare}
     * making up these 2 puzzles. <tt>null</tt> if there is no contact between them
     */
    Puzzle merge(Puzzle other);

    /**
     * Whether this puzzle has been merged with another, basing on their presence on {@link
     * UnitMatrix#matrixP}
     *
  ...
  private class UnitMatrix {
    ...
    /**
     * Concatenate {@link UnitState#BORDER border} units with {@link UnitState#ACCEPTED accepted}
     * one to create polyominos
     *
    ...
    /**
     * Each candidate comes with a list of possibilities, which are in fact puzzles that can couple
     * with that candidate
     */
    ...
    /**
     * Graph of {@link UnitSquare}s to save of each {@link UnitState#ACCEPTED ACCEPTED} {@link
     * UnitSquare} and {@link Polyomino}
     */
    ...
    /**
     * Try to cover the border by trying in order each candidate with each its possibility
     */
    ...
    /**
     * Given current state of {@link #matrixP} and {@link #matrixU}, we find all candidates and
     * register them
     *
     * @see #candidates
    ...
    private void sortCandidates() {
      LOGGER.finer(
          "Sort candidates by " + candidateSorter.toString() + "(" + candidateSorter._d() + ")");
      candidates.sort(candidateSorter._c());
    }

    /**
     * A quick regrouping {@link UnitSquare}s at the border with {@link UnitState#ACCEPTED accepted}
     * one. Not deterministic
     */
    private void quickRegroup() {
      ...
      Comparator<UnitSquare> comparingFamousLevel = (u1, u2) ->
          ...
          // in descendant order
          Integer.compare(proposersRegistry.get(u2)
              .size(), proposersRegistry.get(u1)
              .size());
    ...
    /**
     * Add a new {@link Puzzle} to list of {@link #candidates} for further try. Also figure out its
     * possibilities and sort them by {@link UnitSquarePattern#possibilitySorter}
     *
     * @param puzzle Its connectivities should be already saved in {@link #neighbors}
     * @see #candidates
     */
    ...
    /**
     * Remove the puzzle (a merged one resulted by an {@link Action}) from candidates and its
     * possibilities. Also remove from {@link #neighbors}
     *
    ...
    private class UnitSquare extends Rectangle2D.Double implements Puzzle {
      ...
      UnitSquare(double x, double y, Area area, int i, int j) {
        ...
        if (i < 0 || j < 0) {
          throw new IllegalArgumentException(
              "Virtual coordinates of a Unit Square must not be negative.");
        }
        this._i = i;
        this._j = j;
      }
      ...
      boolean checkContinuousSurface(UnitSquare that) {
        ...
        return AreaTool.getContinuousSurfacesFrom(union)
            .size() == 1;
      }
      ...
      public boolean canMergeWith(Puzzle puzzle) {
        ...
        if (puzzle instanceof UnitSquare) {
          UnitSquare u = (UnitSquare) puzzle;
          return (this.state == UnitState.ACCEPTED || u.state == UnitState.ACCEPTED) && this.touch(
              u);
      ...
      /**
       * @return the number of non {@link UnitState#ACCEPTED ACCEPTED} {@link UnitSquare}s around,
       * divided by 10
       */
      private double getOrphanLevel() {
        ...
        // Top-right
        if (_i > 0 && _j + 1 < matrixU[_i].length
       ...
       *
       * @return this area minus a safe space. Empty if {@link #state} is {@link UnitState#IGNORED
       * IGNORED}
       */
      Area getSafeArea() {
    ...
    /**
     * A sequence of adjacent {@link UnitSquare}s. A polyomino could not be too larger than a {@link
     * Bit2D}. It contains at least one {@link UnitState#ACCEPTED ACCEPTED} {@link UnitSquare}
     *
     * @author Quoc Nhat Han TRAN
     */
...
//			}
      ...
      /**
       * Check if the union is still valid for producing a regular {@link Bit2D}
       *
       * @param newArea raw area of {@link Polyomino} or {@link UnitSquare}
       * @return <tt>true</tt> if fusion does not break limits
       */
      private boolean checkUnionWith(Area newArea) {
        Area union = (Area) this.getRawArea()
            .clone();
        union.add(newArea);
        if (!cutDetails) {
          if (AreaTool.getContinuousSurfacesFrom(union)
              .size() != 1) {
            return false;
          }
        }
        ...
        if (orientation.x == 1 && orientation.y == 0) {
          ...
          return !(union.getBounds2D()
              .getWidth() > maxPWidth)
              && !(union.getBounds2D()
              .getHeight() > maxPLength);
        }
      }

      /**
       * Create a bit from surface of this polyomino. By default, the generated bit will float in
       * the top-left corner. But if an {@link UnitState#ACCEPTED accepted} unit lying on either
       * side of boundary, the generated bit will float toward that side.
       *
       * @return a regular bit if this polyomino has at least one {@link UnitState#ACCEPTED
       * accepted} unit. <tt>null</tt> if bit contains no valid surface
       */
      Bit2D getBit2D() {
        if (this.isEmpty()) {
          return null;
      ...
      Area roundUp(Area bitArea) {
        return AreaTool.getContinuousSurfacesFrom(bitArea)
            .stream()
            .filter(Objects::nonNull)
            .filter(area
      ...
      private Vector2 getBitOrientation() {
        if (rawArea.getBounds2D()
            .getWidth() >= rawArea.getBounds2D()
            .getHeight()) {
       ...
       * @param floatpos    either "top-left", "top-right", "bottom-left" or "bottom-right"
      ...
      /**
       * The bit surface should be limited inside boundary of this polyomino minus a minimum margin.
       * The margin should run along opposite border of floating position. If the bit is bigger than
       * the polyomino, the margin's width will be equal to pattern's parameter. Otherwise, the
       * margin's width would be the difference between bit's and polyomino's size.
       *
       * @param floatingPosition either "top-left", "top-right", "bottom-left" or "bottom-right"
      ...
      private Area getLimArea(String floatingPosition, Vector2 bitOrientation) {
        ...
        if (boundary.width <= bitHorizontalLength) {
          ...
          // "horizontal margin"
          horizontalMarginAroundBit = (double) config.get(HORIZONTAL_MARGIN)
              .getCurrentValue();
        } else {
        ...
        if (boundary.height <= bitVerticalLength) {
          // We will put in a margin whose width is equal to pattern's parameter
          // "vertical margin"
          verticalMarginAroundBit = (double) config.get(VERTICAL_MARGIN)
              .getCurrentValue();
        } else {
          // Margin will be difference between boundary's size and bit's
    ...
    private class Action {
       ...
       * @throws TooDeepSearchException   if we had done more than demanded
       * @throws IllegalArgumentException if this action has been done before (with <tt>target</tt>
       *                                  as trigger and <tt>trigger</tt> as target)
       */
      Action(Action parent, Puzzle trigger, Puzzle target)
        ...
        if (parent != null) {
          if (parent.getChildren()
              .parallelStream()
              .anyMatch(c -> c.hasTriedToMerge(trigger, target))) {
      ...
      /**
       * Apply this action to the current state of {@link UnitMatrix#matrixP}. Register the result
       * into {@link UnitMatrix#matrixP}
       *
      ...
      /**
       * Revert this action. Unregister the merged polyominos. Remove result from {@link
       * UnitMatrix#candidates} and {@link UnitMatrix#neighbors}
       */
      void undo() {
      ...
      /**
       * Two {@link Action} are equal if their targets and triggers are asymetrically identical
       */
      @Override
      public boolean equals(Object arg0) {
