 ...
 * of paving. Therefore, it requires auto-optimization task to complete. Note that this pattern does
 * not always return a result because once it can not fill a separated zone of the generated part,
...
public class EconomicPattern extends PatternTemplate {
  ...
  /**
   * Contains all offsets for trying to rotate this layer, in order to not have the same rotation
  ...
  /**
   * Contains all offsets for trying to dislocate the first bit of a line following x-axe.
   */
  ...
  /**
   * This will try to pave bits into the layer in the best way possible. The process should be
   * executed consequently because the rotation of each layer depends on the previous one
   *
   * @param layer target
  ...
  private Pavement pave(int layerNumber, Area area) {
    ...
    this.bitsLengthSpace = (double) config.get("bitsLengthSpace")
        .getCurrentValue();
    this.bitsWidthSpace = (double) config.get("bitsWidthSpace")
        .getCurrentValue();
      ...
      while (!unpavedZone.isEmpty()) {
        ...
        if (thisBand == null) {
          ...
          if (lastBand == null) {
            ...
            if (lastBand.firstElement()
                .getWidth() == CraftConfig.bitWidth) {
  ...
  private Vector<Bit2D> fillBand(Area band, double bandWidth) {
    // A little tweak
    if (band.getBounds2D()
        .getHeight() < CraftConfig.suckerDiameter
        || band.getBounds2D()
        .getWidth() < CraftConfig.suckerDiameter) {
      return null;
    }
    ...
    for (double trialLengthOffset : trialLengthOffsets) {
      ...
      double originY =
          bandOuterRect.y + CraftConfig.bitWidth / 2, thisBitLength = CraftConfig.lengthFull;
      boolean essay = true;
      // Start
  ...
  private void setupTrialLengthOffsets() throws ClassCastException, UnsupportedOperationException {
    List<java.lang.Double> a = (List<java.lang.Double>) config.get("trialLengthRatioOffsets")
        .getCurrentValue();
    // Adapt values
  ...
  private void setupTrialHeightOffsets() {
    List<java.lang.Double> a = (List<java.lang.Double>) config.get("trialHeightRatioOffsets")
        .getCurrentValue();
  ...
  private void setupTrialRotations(int layerNum) {
    List<java.lang.Double> a = (List<java.lang.Double>) config.get("trialDiffAngles")
        .getCurrentValue();
   ...
   * @return the matrix to move the <tt>area</tt> to the origin of coordinate system
   */
