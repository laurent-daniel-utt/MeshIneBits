...
public class EconomicPattern extends PatternTemplate {
    ...
    private Vector<Bit2D> fillBand(Area band, double bandWidth) {
        ...
        for (double trialLengthOffset : trialLengthOffsets) {
            ...
            double originY = bandOuterRect.y + CraftConfig.bitWidth / 2, thisBitLength = CraftConfig.bitLengthNormal;
            boolean essay = true;
            // Start
            while (!unpavedSpace.isEmpty()) {
                ...
                Vector2 origin = new Vector2(unpavedSpaceRect.x + CraftConfig.bitLengthNormal / 2, originY);
                ...
                if (!DetectorTool.checkIrregular(newBit)) {
                    ...
                    thisBitLength = CraftConfig.bitLengthNormal;
                } else {
                    // If no, it means we failed
                    if (thisBitLength == CraftConfig.bitLengthNormal) {
                        ...
                        if (bandPavement.isEmpty()) {
                            ...
                            if (lastBit.getLength() == CraftConfig.bitLengthNormal / 2) {
                                ...
                                double lastX = lastBit.getOrigin().x - CraftConfig.bitLengthNormal / 2;
                                ...
                                thisBitLength = CraftConfig.bitLengthNormal / 2;
    ...
    private void setupTrialLengthOffsets() throws ClassCastException, UnsupportedOperationException {
        ...
        for (int i = 0; i < a.size(); i++) {
            trialLengthOffsets[i] = a.get(i) * CraftConfig.bitLengthNormal;
        }
    }
