...
public abstract class PatternTemplate implements Serializable, Cloneable {
   ...
   *
   * @param patternConfig all given parameters will be filtered again before applying. Those which
   *                      do not appear in the default configuration of template will be discarded.
   */
  public PatternTemplate(PatternConfig patternConfig) {
    ...
    for (String configName : config.keySet()) {
      if (patternConfig.containsKey(configName)) {
        Object currentValue = config.get(configName)
            .getCurrentValue(),
            newValue = patternConfig.get(configName)
                .getCurrentValue();
        if (currentValue.getClass()
  ...
  /**
   * Auto-optimization. Also a constructor who considers completely the layer's boundary.
   *
